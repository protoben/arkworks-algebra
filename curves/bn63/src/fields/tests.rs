...
fn test_fq_repr_from() {
    assert_eq!(BigInteger64::from(100u64), BigInt::new([100]));
}

#[test]
fn test_fq_repr_is_odd() {
    assert!(!BigInteger64::from(0u64).is_odd());
    assert!(BigInteger64::from(0u64).is_even());
    assert!(BigInteger64::from(1u64).is_odd());
    assert!(!BigInteger64::from(1u64).is_even());
    assert!(!BigInteger64::from(324834872u64).is_odd());
    assert!(BigInteger64::from(324834872u64).is_even());
    assert!(BigInteger64::from(324834873u64).is_odd());
    assert!(!BigInteger64::from(324834873u64).is_even());
}

#[test]
fn test_fq_repr_is_zero() {
    assert!(BigInteger64::from(0u64).is_zero());
    assert!(!BigInteger64::from(1u64).is_zero());
...
fn test_fq_repr_num_bits() {
    let mut a = BigInteger64::from(0u64);
    assert_eq!(0, a.num_bits());
    a = BigInteger64::from(1u64);
    for i in 1..257 {
...
#[test]
fn test_fq_num_bits() {
    assert_eq!(Fq::MODULUS_BIT_SIZE, 63);
}

// XXX Only need this for FFT
#[test]
#[ignore]
...
#[test]
fn test_fq_ordering() {
    // BigInteger64's ordering is well-tested, but we still need to make sure the
    // Fq elements aren't being compared in Montgomery form.
    for i in 0..100u64 {
        assert!(Fq::from(BigInteger64::from(i + 1)) > Fq::from(BigInteger64::from(i)));
    }
}
...
fn test_fq_legendre() {
    ...
    assert_eq!(
        QuadraticResidue,
        Fq::from(BigInteger64::from(4u64)).legendre()
    );
    assert_eq!(
        QuadraticNonResidue,
        Fq::from(BigInteger64::from(5u64)).legendre()
    );
}
